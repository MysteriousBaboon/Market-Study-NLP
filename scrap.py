...
def scrape_page(url):
    """
    Function used to scrape a company page and get all reviews
    """
    global api_df
    url = domain + url["href"]
    ...
    comp_name = soup.find("span", {"class": "multi-size-header__big"}).text
...
def scrap_db():
    """
    Function used to Scrape every reviews on the site.
    """
    ...
    for category in category_urls:
        for i in range(1, 11):
            number = f"?page={i}"
            page = requests.get(domain + "/categories/" + category + number)
            soup = BeautifulSoup(page.text, 'lxml')
...
def scrap_api(field: str, location: str):
    ...
    page = requests.get(domain + "/categories/" + field + f"?location={location}")
